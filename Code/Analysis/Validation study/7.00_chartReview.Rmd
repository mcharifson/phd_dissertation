...
true_df_clean <-  redcap_df %>% 
  filter(secondary_review_final != 1 | is.na(secondary_review_final)) %>% 
...
print(paste('There are', length(which(review_notes_flag$uf_notes_flag=='y')), 'patients who have an uncertain UF dx from chart review.'))

print(paste('There are', length(which(review_notes_flag$endo_notes_flag=='y')), 'patients who have an uncertain Endo dx from chart review.'))
```
...
prev_endo = length(which(endo_final$endo_algorithm=='Incident'))/dim(pat_cohort)[1]
prev_uf = length(which(uf_final$uf_algorithm=='Incident'))/dim(pat_cohort)[1]

performance_stats <- function(df) {
  # make table
  perform_df <- tibble(ppv = numeric(), npv = numeric(), sens = numeric(), spec = numeric())
  # get metrics
  perform_df[1, 1] <- ppv(df, chart_case, ehr_case)$.estimate
  perform_df[1, 2] <- npv(df, chart_case, ehr_case)$.estimate
  perform_df[1, 3] <- sens(df, chart_case, ehr_case)$.estimate
  perform_df[1, 4] <- spec(df, chart_case, ehr_case)$.estimate
  # print table
  return(perform_df)
}
```
...
```{r}
t <- outcome_tbl %>% filter(mrn %in% pred_df$mrn)

# putative incident cases + putative controls
uf_ids <- t$mrn[which(t$false_negative==FALSE & t$endo_icd_outcome_status=='Control' & !t$uf_gold_date_flag)]
#pred_df$mrn[which(pred_df$final_outcome_status %in% c('Incident Endo only', 'Endo unconfirmed', 'Neither incident'))]

# putative incident cases + putative controls
endo_ids <- t$mrn[which(t$false_negative==FALSE & t$uf_icd_outcome_status=='Control' & !t$endo_gold_date_flag)]
#pred_df$mrn[which(pred_df$final_outcome_status %in% c('Incident Endo only', 'Endo unconfirmed', 'Neither incident'))]
```

## Ability to identify cases {.tabset}

```{r}
bin_table <- data.frame(approach=rep(c(1,2,3), each=2), outcome=rep(c('UF', 'Endo'), 3),
                        PPV = rep(NA,6), NPV = rep(NA,6), Kappa = rep(NA,6))
```
...
perform_df <- performance_stats(df)
# add to bin_table
bin_table$PPV[1] <- perform_df$ppv
bin_table$NPV[1] <- perform_df$npv
bin_table$Kappa[1] <- as.numeric(cohen.kappa(x=df)[1])
# print confusion matrix
kable(addmargins(table(df$chart_case, df$ehr_case)), col.names = binary_labels) %>% 
  kable_styling() %>%
  add_footnote(c(paste0("PPV = ", round(perform_df$ppv, 3)),
                 paste0("NPV = ", round(perform_df$npv, 3)),
                 paste0("Sensitivity = ", round(perform_df$sens, 3)),
                 paste0("Specificity = ", round(perform_df$spec, 3)),
                 paste0("Kappa = ", round(as.numeric(cohen.kappa(x=df)[1]), 3))),
                 notation="number")
```
...
perform_df <- performance_stats(df)
# add to bin_table
bin_table$PPV[2] <- perform_df$ppv
bin_table$NPV[2] <- perform_df$npv
bin_table$Kappa[2] <- as.numeric(cohen.kappa(x=df)[1])
# print confusion matrix
kable(addmargins(table(df$chart_case, df$ehr_case)), col.names = binary_labels) %>% 
  kable_styling() %>%
  add_footnote(c(paste0("PPV = ", round(perform_df$ppv, 3)),
...
### ICD + diagnostic procedures

```{r}
# define cases from diagnostic procedures
pred_df$uf_any_confirmed <- case_when(pred_df$uf_imaging_confirmed | pred_df$uf_surgery_confirmed~TRUE, 
                                      TRUE~FALSE)
pred_df$endo_any_confirmed <- case_when(pred_df$endo_imaging_confirmed | pred_df$endo_surgery_confirmed~TRUE, 
                                        TRUE~FALSE)
```
...
```{r}
# select variables
df <- validate_cases(uf_ids, 'uf_any_chart', 'uf_any_confirmed')
perform_df <- performance_stats(df)
# add to bin_table
bin_table$PPV[3] <- perform_df$ppv
bin_table$NPV[3] <- perform_df$npv
bin_table$Kappa[3] <- as.numeric(cohen.kappa(x=df)[1])
# print confusion matrix
kable(addmargins(table(df$chart_case, df$ehr_case)), col.names = binary_labels) %>% 
  kable_styling() %>%
  add_footnote(c(paste0("PPV = ", round(perform_df$ppv, 3)),
...
```{r}
# select variables
df <- validate_cases(endo_ids, 'endo_any_chart', 'endo_any_confirmed')
perform_df <- performance_stats(df)
# add to bin_table
bin_table$PPV[4] <- perform_df$ppv
bin_table$NPV[4] <- perform_df$npv
bin_table$Kappa[4] <- as.numeric(cohen.kappa(x=df)[1])
# print confusion matrix
kable(addmargins(table(df$chart_case, df$ehr_case)), col.names = binary_labels) %>% 
  kable_styling() %>%
  add_footnote(c(paste0("PPV = ", round(perform_df$ppv, 3)),
...
perform_df <- performance_stats(df)
# add to bin_table
bin_table$PPV[5] <- perform_df$ppv
bin_table$NPV[5] <- perform_df$npv
bin_table$Kappa[5] <- as.numeric(cohen.kappa(x=df)[1])
# print confusion matrix
kable(addmargins(table(df$chart_case, df$ehr_case)), 
      col.names = c('Case (pred)', 'No Dx (pred)', 'Sum')) %>% 
  kable_styling() %>%
  add_footnote(c(paste0("PPV = ", round(perform_df$ppv, 3)),
                 paste0("NPV = ", round(perform_df$npv, 3)),
                 paste0("Sensitivity = ", round(perform_df$sens, 3)),
...
perform_df <- performance_stats(df)
# add to bin_table
bin_table$PPV[6] <- perform_df$ppv
bin_table$NPV[6] <- perform_df$npv
bin_table$Kappa[6] <- as.numeric(cohen.kappa(x=df)[1])
# print confusion matrix
kable(addmargins(table(df$chart_case, df$ehr_case)), 
      col.names = c('Case (pred)', 'No Dx (pred)', 'Sum')) %>% 
  kable_styling() %>%
...
```{r}
multi_table <- data.frame(approach=rep(c(1,2,3), each=2), outcome=rep(c('UF', 'Endo'), 3),
                        PPV = rep(NA,6), NPV = rep(NA,6), Kappa = rep(NA,6))
```
...
perform_df <- performance_stats(df)
# add to multi_table
multi_table$PPV[1] <- perform_df$ppv
multi_table$NPV[1] <- perform_df$npv
multi_table$Kappa[1] <- as.numeric(cohen.kappa(x=df)[1])
# print confusion matrix
kable(addmargins(table(df$chart_case, df$ehr_case)), col.names = full_labels) %>% 
  kable_styling() %>%
  add_footnote(c(paste0("PPV = ", round(perform_df$ppv, 3)),
                 paste0("NPV = ", round(perform_df$npv, 3)),
                 paste0("Sensitivity = ", round(perform_df$sens, 3)),
...
perform_df <- performance_stats(df)
# add to multi_table
multi_table$PPV[2] <- perform_df$ppv
multi_table$NPV[2] <- perform_df$npv
multi_table$Kappa[2] <- as.numeric(cohen.kappa(x=df)[1])
# print confusion matrix
kable(addmargins(table(df$chart_case, df$ehr_case)), 
      col.names = c('Incident (pred)', 'Prevalent (pred)', 'No Dx (pred)', 'Sum')) %>% 
  kable_styling() %>%
  add_footnote(c(paste0("PPV = ", round(perform_df$ppv, 3)),
                 paste0("NPV = ", round(perform_df$npv, 3)),
                 paste0("Sensitivity = ", round(perform_df$sens, 3)),
...
Uterine fibroids
```{r}
pred_df$uf_any_confirmed <- case_when(pred_df$uf_imaging_confirmed | pred_df$uf_surgery_confirmed~'Incident', 
                                      pred_df$uf_icd_confirmed~'Prevalent', 
                                      TRUE~'No Dx')
# select variables
df <- validate_cases(uf_ids, 'uf_gold_dx', 'uf_any_confirmed')
df$chart_case <- factor(df$chart_case, levels=c('Incident', 'Prevalent', 'No Dx'))
df$ehr_case <- factor(df$ehr_case, levels=c('Incident', 'Prevalent', 'No Dx'))
perform_df <- performance_stats(df)
# add to multi_table
multi_table$PPV[3] <- perform_df$ppv
multi_table$NPV[3] <- perform_df$npv
multi_table$Kappa[3] <- as.numeric(cohen.kappa(x=df)[1])
# print confusion matrix
kable(addmargins(table(df$chart_case, df$ehr_case)), col.names = full_labels) %>% 
  kable_styling() %>%
  add_footnote(c(paste0("PPV = ", round(perform_df$ppv, 3)),
...
Endometriosis
```{r}
pred_df$endo_any_confirmed <- case_when(pred_df$endo_imaging_confirmed | pred_df$endo_surgery_confirmed~'Incident', 
                                        pred_df$endo_icd_confirmed~'Prevalent', 
                                        TRUE~'No Dx')
# select variables
df <- validate_cases(endo_ids, 'endo_gold_dx', 'endo_any_confirmed')
df$chart_case <- factor(df$chart_case, levels=c('Incident', 'Prevalent', 'No Dx'))
df$ehr_case <- factor(df$ehr_case, levels=c('Incident', 'Prevalent', 'No Dx'))
perform_df <- performance_stats(df)
# add to multi_table
multi_table$PPV[4] <- perform_df$ppv
multi_table$NPV[4] <- perform_df$npv
multi_table$Kappa[4] <- as.numeric(cohen.kappa(x=df)[1])
# print confusion matrix
kable(addmargins(table(df$chart_case, df$ehr_case)), col.names = full_labels) %>% 
  kable_styling() %>%
  add_footnote(c(paste0("PPV = ", round(perform_df$ppv, 3)),
...
perform_df <- performance_stats(df)
# add to multi_table
multi_table$PPV[5] <- perform_df$ppv
multi_table$NPV[5] <- perform_df$npv
multi_table$Kappa[5] <- as.numeric(cohen.kappa(x=df)[1])
# print confusion matrix
kable(addmargins(table(df$chart_case, df$ehr_case)), col.names = full_labels) %>% 
  kable_styling() %>%
  add_footnote(c(paste0("PPV = ", round(perform_df$ppv, 3)),
...
perform_df <- performance_stats(df)
# add to multi_table
multi_table$PPV[6] <- perform_df$ppv
multi_table$NPV[6] <- perform_df$npv
multi_table$Kappa[6] <- as.numeric(cohen.kappa(x=df)[1])
# print confusion matrix
kable(addmargins(table(df$chart_case, df$ehr_case)), col.names = full_labels) %>% 
  kable_styling() %>%
  add_footnote(c(paste0("PPV = ", round(perform_df$ppv, 3)),
...
# desperate checks to find any meaning in life
test <- true_df_clean %>%
    filter(mrn %in% endo_ids) %>%
    left_join(pred_df, by='mrn') %>%
    left_join(endo_cohort, by='pat_id')

summary(CreateTableOne(var='time_to_first_dx', strata='endo_gold_dx', data=test),
        nonnormal = 'time_to_first_dx')

hist(as.numeric(test$time_to_first_dx[which(test$endo_gold_dx == 'Incident')]))
hist(as.numeric(test$time_to_first_dx[which(test$endo_gold_dx == 'Prevalent')]))

no_match <- test %>% 
  filter(endo_gold_dx == 'No Dx' & endo_algorithm == 'Incident') %>% 
  select(mrn)

true_df_clean %>% 
  filter(mrn %in% no_match$mrn) %>%
  left_join(adeno %>% left_join(pat_cohort %>% select(pat_id, pat_mrn_id), by='pat_id'), 
            by=c('mrn' = 'pat_mrn_id')) %>%
  select(contains('endo'), notes_final, contains('adeno')) %>%
  View()

endo_cohort %>% 
  left_join(pat_cohort %>% select(pat_id, pat_mrn_id), by='pat_id') %>%
  filter(pat_mrn_id %in% no_match$mrn) %>% 
  mutate(first_dx_date = as.Date(first_dx_date)) %>%
  summary()
```
...
```{r}
summary(as.integer(uf_dates_comp$true_v_pred_date_gap/30))

print(paste('The total misattributed person-time due to incorrect earliest UF dx date from ICD-10 in validation study is', round(sum(as.integer(uf_dates_comp$true_v_pred_date_gap/30)),2), 'person-months.'))
```
...
print(paste('The total misattributed person-time due to incorrect earliest dx Endo date from ICD-10 in validation study is', round(sum(as.integer(endo_dates_comp$true_v_pred_date_gap/30), na.rm=T),2), 'person-months.'))
```
...
```{r}
CreateTableOne(vars='both_gold_dx', data=both_df_clean)
```
