...
procedure_hints <- uf_all_long
setDT(procedure_hints)
# indexing which rows are procedures
procedure_hints$is_procedure <- grepl('Imaging|Surgery', procedure_hints$event_type)
# indexing which rows are dx (ignore imaging Dx for now)
procedure_hints$is_diagnosis <- grepl('Diagnosis|Problem|List|History', procedure_hints$event_type) & !grepl('Imaging',procedure_hints$event_type)
# rearrange so if a procedure and dx happen on the same day the procedure goes first
procedure_hints <- arrange(procedure_hints, pat_id, date, is_diagnosis, is_procedure)
# get the index of the next row that is a dx when the row is a procedure
procedure_hints[,next_dx_index:=zoo::na.locf(NA^(!is_diagnosis)*.I, fromLast=T, na.rm=F)*NA^(!is_procedure), by=pat_id]
# find time difference between the date of procedure and the next Dx date
procedure_hints <- procedure_hints[,next_dx_time_interval:=difftime(date[next_dx_index], date, units="days")]
procedure_hints <- procedure_hints[,next_dx_name:=event_name[next_dx_index]]
procedure_hints <- procedure_hints[,next_dx_type:=event_type[next_dx_index]]
# convert to data frame
procedure_hints <- as.data.frame(procedure_hints) %>%
...
`r length(which(procedure_hints$next_dx_time_interval==0))` diagnosis codes are on the same date as a procedure.
...
```{r surgery_hints}
surgery_hints <- uf_pat_unique %>%
  left_join(procedure_hints %>% filter(method == 'Surgery'), 
            by='pat_id') %>%
  group_by(pat_id) %>%
  summarise(any_surgery = (sum(is_procedure, na.rm=TRUE)>0),
            dx_after_6months = (sum(confirmed_6months, na.rm=TRUE)>0),
            dx_after_prevalent = (sum(confirmed_ever_prevalent, na.rm=TRUE)>0),
            date_first_confirmed_surgery = min(date[which(confirmed_6months |
                                                            confirmed_ever_prevalent)], 
                                               na.rm=TRUE))
#summary(surgery_hints)
```

Derive imaging hints for each patients
```{r imaging_hints}
imaging_hints <- uf_pat_unique %>%
  left_join(procedure_hints %>% filter(method == 'Imaging'), 
            by='pat_id') %>%
  group_by(pat_id) %>%
  summarise(any_imaging = (sum(is_procedure, na.rm=TRUE)>0),
            dx_after_6months = (sum(confirmed_6months, na.rm=TRUE)>0),
            dx_after_prevalent = (sum(confirmed_ever_prevalent, na.rm=TRUE)>0),
            date_first_confirmed_imaging = min(date[which(confirmed_6months |
                                                            confirmed_ever_prevalent)], 
                                               na.rm=TRUE))
#summary(imaging_hints)
```
...
procedure_hints$location <- grepl(uf_loc_names, tolower(procedure_hints$event_name))
## add to other hints
other_hints <- uf_pat_unique %>%
  left_join(procedure_hints %>%
              group_by(pat_id) %>%
              summarise(any_location = (sum(!is.na(location))>0),
                        date_first_location = min(date[which(!is.na(location))], na.rm=TRUE)),
            by='pat_id')
```
...
procedure_hints$pregnancy_related <- (!grepl("History", procedure_hints$event_type) &
                                        grepl(uf_pregnancy, tolower(procedure_hints$event_name)))
# join to other hints
...
uf_hx_code <- uf_endo_cohort %>% 
  ...
  mutate(hx_code_margin = difftime(first_hx_code, first_dx_date, unit='days'),
         confident_prevalent = (uf_hx_code$hx_code_margin<=60))

# join to other hints
other_hints <- other_hints %>%
  left_join(procedure_hints %>%
              group_by(pat_id) %>%
              summarise(confident_prevalent = (sum(confident_prevalent)>0)), 
            by='pat_id')
```
