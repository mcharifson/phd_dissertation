...
pat_cohort_new <- pat_cohort %>%
  ...
  mutate(uf_dx = case_when(!is.na(uf_first_dx_date)~TRUE, TRUE~FALSE),
         ...
         final_outcome_status = case_when((uf_dx & endo_dx) & uf_first_dx_date < endo_first_dx_date~'UF',
                                   (uf_dx & endo_dx) & uf_first_dx_date > endo_first_dx_date~'Endo',
                                   (uf_dx & endo_dx) & uf_first_dx_date == endo_first_dx_date~'Both',
                                   uf_dx & !endo_dx~'UF',
                                   !uf_dx & endo_dx~'Endo',
                                   !uf_dx & !endo_dx~'Control'),
         first_outcome_dx_date = case_when(final_outcome_status %in% c('UF', 'Both')~uf_first_dx_date,
                                           final_outcome_status == 'Endo'~endo_first_dx_date),
         outcome_before_censor = case_when(is.na(final_censor_date)~TRUE,
         ...
         final_censor_type_outcome = case_when(final_outcome_status != 'Control' & outcome_before_censor~'Case',
                                              final_outcome_status %in% c('UF', 'Both') &
                                                final_censor_type==uf_first_confirm_type~'Case',
                                              final_outcome_status == 'Endo' &
...
pat_cohort_symptoms <- pat_cohort_new %>%
  inner_join(first_symptom %>%
              filter(time_group %in% c('at baseline', 'during follow-up') & !flag) %>%
...
pat_cohort_symptoms$ttd <- as.numeric(difftime(pat_cohort_symptoms$final_censor_date_outcome,
                                               pat_cohort_symptoms$first_symptom_date,
                                               unit='days'))/30
## show table for ttd by outcome status
summary(tableby(final_outcome_status~ttd, data=pat_cohort_symptoms), 
        labelTranslations= list('ttd'='Time to diagnosis (months)'))
```
...
pat_cohort_symptoms %>%
  ...
  summary(., labelTranslations= list('ttd'='Time to diagnosis (months)'))
```
...
false_negatives <- symptoms %>%
  inner_join(pat_cohort_new %>% 
               select(pat_id, final_censor_date_outcome), 
            by='pat_id') %>%
  filter(dx_date <= final_censor_date_outcome) %>%
  group_by(pat_id) %>%
  ...
  filter(!pat_id %in% pat_cohort_new$pat_id[which(pat_cohort_new$final_censor_type_outcome == 'Case')])
```
