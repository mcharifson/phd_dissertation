...
# Eligible cohort
ch1_cohort %>% distinct(pat_id) %>% nrow()
# Outcome could not be ascertained
ch1_cohort %>% 
  # keep only confident cases or controls (not those with hesitant positives)
  filter(uf_dx | endo_dx | (is.na(uf_confident_incident) & is.na(endo_confident_incident))) %>% 
...
ch1_cohort %>% 
  ...
  distinct(pat_id) %>% nrow()
# Is excluded due outcome status
ch1_cohort %>% filter(!uf_confident_incident & !endo_confident_incident) %>% distinct(pat_id) %>% nrow()
# Is excluded due to exposure data
length(which(!is.na(ch1_cohort$addr_NA_reason) | ch1_cohort$site_baseline == 'FLORIDA'))
```
...
DiagrammeR::grViz("digraph {
  ...
  rec2 [label = 'Complete Outcome Data (n=73,481)']
  rec3 [label =  'Complete Exposure Data (n=71,243)']
  rec4 [label = 'Final sample size (n=71,243)']
  rec5 [label = 'Excluded Patients (n=3,152)']
  rec6 [label = 'Outcome status could \n not be ascertained (n=914)']
...
```{r all_data}
all_data <- ch1_cohort %>% 
  left_join(pat_w_covars %>% select(-site_baseline), by='pat_id') %>%
  ...
  left_join(ttd_symptoms %>% select(pat_id, ttd, first_symptom_date, first_symptom_icd10s, first_symptom_dx_name), by='pat_id') %>%
  ...
  mutate(uf_person_years = case_when(is.na(final_censor_date)~
                                    difftime(last_enc_date, index_enc_date,units='days'),
...
write.csv(all_data, 'Data/Derived/all_data_noImpute_20240613.csv', row.names = FALSE)
```
...
write.csv(ch1_final, 'Data/Derived/Chapter 1/final_dataset_20240514.csv', row.names = FALSE)
write.csv(ch1_final, 'Data/Derived/Chapter 1/sample_flags_20240529.csv', row.names = FALSE)
```
...
ch1_all_cases <- ch1_cohort %>% 
  filter(is.na(addr_NA_reason)) %>%
  left_join(pat_w_covars %>% select(-site_baseline), by='pat_id') %>%
  left_join(pat_w_exposure %>% select(-addr_NA_reason), by='pat_id') %>%
  mutate(ICE_quartile = ntile(ICE_RI, 4),
         ICE_quartile = factor(ICE_quartile, labels=c('Q1', 'Q2', 'Q3', 'Q4'))) %>%
  mutate(uf_person_years = case_when(is.na(final_censor_date)~
                                    difftime(last_enc_date, index_enc_date,units='days'),
