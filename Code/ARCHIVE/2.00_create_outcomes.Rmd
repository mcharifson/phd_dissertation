...
endo_dx_unique <- uf_endo_cohort %>% 
  filter(grepl('N80', icd10) & !grepl('N80.03', icd10) & !is.na(dx_date)) %>%
  rename(date=dx_date) %>%
  select(-referral_id, -icd10, -dx_date_filled, -pat_enc_csn_id) %>%
  group_by(pat_id, date) %>%
  summarise(event_name = paste0(unique(dx_name), collapse=', '),
...
# 2.2. Explore low confidence cases and identify subset who need manual chart review

# 2.3. Explore Referral into OB/GYN (for index encounter or otherwise)

```{r}
length(which(uf_endo$dx_type == 'Referral Diagnosis'))
```
