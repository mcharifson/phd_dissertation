...
enc_inpatient <- read.csv('Data/Epic/table2_enc_inpatient.csv')
enc_outpatient1 <- read.csv('Data/Epic/table2_enc_outpatient1.csv')
enc_outpatient2 <- read.csv('Data/Epic/table2_enc_outpatient2.csv')
enc_outpatient3 <- read.csv('Data/Epic/table2_enc_outpatient3.csv')
enc_telemedicine <- read.csv('Data/Epic/table2_enc_telemedicine.csv')
...
enc_ip_op <- rbind(enc_inpatient, 
                   enc_outpatient1[,-16], enc_outpatient2[,-16], enc_outpatient3[,-16],
                   enc_telemedicine)
```
...
```{r regroup_sites}
## regroup the loc_name variable
enc_ip_op <- enc_ip_op %>%
  mutate(loc_info = tolower(paste0(loc_name, ';', dep_external_name)),
         old_loc_name = loc_name,
         loc_name = case_when(grepl('telemed|telehealth', loc_info) | enc_type == 'Telemedicine'~ 'TELEMEDICINE',
                              grepl('tisch|kimmel|trinity|nyu langone orthopedic hospital', loc_info)~'TISCH HOSPITAL',
                              grepl('family health center', loc_info) ~ 'FAMILY HEALTH CENTERS',
                              grepl('brooklyn', loc_info)~'NYU LANGONE BROOKLYN',
                              grepl('long island|winthrop', loc_info)~'NYU LANGONE HOSPITAL - LONG ISLAND',
                              TRUE~'OTHER'))

#table(enc_ip_op$loc_name)

#enc_ip_op %>%
  #filter(loc_name == 'NYU LANGONE BROOKLYN') %>%
  #filter(loc_name == 'FAMILY HEALTH CENTERS') %>%
  #filter(loc_name == 'TELEMEDICINE') %>%
  #filter(loc_name == 'NYU LANGONE ORTHOPEDIC HOSPITAL') %>%
  #filter(loc_name == 'OUTPATIENT CLINIC') %>%
...
ed_all <- enc_ed %>%
  ...
  rename(provider_specialty = longest_prov_specialty, loc_name = first_loc_name)
...
all_enc_FU <- all_enc %>% filter(enc_date >= index_enc_date)
```

To be eligible for the cohort, each patient must have at least two relevant care encounter in the first two years of follow-up (Jan 2018-Dec 2020) to consider having maintained contact with NYU, which establishes that they have two consecutive (August 2016-2017 and 2018) years of relevant care. This is to remove temporary patients from the cohort. 

```{r fu_past_baseline}
fu_past_baseline <- all_enc_FU %>%
  filter(enc_date >= '2018-01-01' & enc_date <= '2020-12-31') %>%
  group_by(pat_id) %>%
  filter(n()>=2) %>%
  distinct(pat_id)
```
...
```{r hcu_baseline}
hcu <- all_enc %>%
  filter(enc_date > index_enc_date & enc_date <= '2017-12-31') %>%
  group_by(pat_id) %>%
...
The goal here is to know out of Tisch, Brooklyn, FHC and Long Island, which site is the most popular for a given patient. Despite this, most patients will predominantly seek care at outpatient clinics, but we are interested in which hospital would service them if they are to go to a hospital. Therefore as long as they have any visits to a hospital site, we will use this as their most common site. If all of their visits were to outpatient clinis, then they will be assigned 'OTHER'. 

NOTE: this is currently only for relevant encounters but we could expand to all encounters if requested. 

```{r site}
## grab most common site even if its other
site_all <- all_enc %>%
  filter(pat_id %in% pat_unique_final$pat_id) %>%
  mutate(group = ifelse(enc_date <= '2019-12-31', 'baseline', 'follow-up')) %>% 
  count(pat_id, group, loc_name) %>%
  group_by(pat_id, group) %>%
  slice(which.max(n)) %>%
  ungroup()

## grab most common excluding other
site <- all_enc %>%
  filter(pat_id %in% pat_unique_final$pat_id) %>%
  filter(loc_name != 'OTHER') %>%
  mutate(group = ifelse(enc_date <= '2019-12-31', 'baseline', 'follow-up')) %>% 
  count(pat_id, group, loc_name) %>%
  group_by(pat_id, group) %>%
  slice(which.max(n)) %>%
...
site <- site %>% 
  ## join for missing baseline
  bind_rows(site_all %>% filter(!pat_id %in% site$pat_id[which(site$group == 'baseline')] & group == 'baseline')) %>%
  ## join for missing follow-up
  bind_rows(site_all %>% filter(!pat_id %in% site$pat_id[which(site$group == 'follow-up')] & group == 'follow-up')) %>%
  # regroup LI community hospital with LI
  mutate(loc_name = ifelse(loc_name == 'LONG ISLAND COMMUNITY HOSPITAL', 'NYU LANGONE HOSPITAL - LONG ISLAND', loc_name))
```

```{r site_table}
CreateTableOne(data=site, vars='loc_name', strata='group', includeNA=TRUE)
```

```{r site_sankey, warning=FALSE, out.width='100%'}
site %>%
  select(-n) %>%
  pivot_wider(names_from = group, values_from = loc_name) %>%
  count(baseline, `follow-up`) %>%
  mutate(percent = (n/dim(pat_unique_final)[1])*100) %>%
  arrange(baseline, desc(percent)) %>%
  ggplot(data = .,
       aes(axis1 = baseline,   # First variable on the X-axis
           axis2 = `follow-up`, # Second variable on the X-axis
           y = percent)) +
...
pat_unique_final <- pat_unique_final %>%
  ...
  left_join(site %>% select(-n) %>% pivot_wider(names_from = group, values_from=loc_name), by='pat_id')
```
