...
ethnicity <- subset_clean(ethnicity) %>% select(-raw_ethnicity_background) %>%
  mutate(fhc_ethnicity = ifelse(grepl('Unreported|Unmapped', fhc_value), NA, fhc_value))
race <- subset_clean(race) %>% select(-race_name) %>%
  mutate(fhc_race_group = na_if(fhc_race_group, 'Unreported/Refused to Report Race'))
...
# remove one patient with both public and private --> private
insur_clean_unique <- insur_clean_unique[-which(insur_clean_unique$pat_id == 'Z5120947' &
                                                  insur_clean_unique$insur_status == 'public'),]
...
```{r merge_race_ethnic}
race_ethnic_final <- race_ethnic %>% 
  group_by(pat_id) %>% 
  filter(n()==1 | (n()>1 & !is.na(race_final))) %>% 
  group_by(pat_id) %>%
  reframe(race_groups = case_when(n_distinct(race_final)>1~paste0(unique(race_final),collapse = ', '),
                                   TRUE~NA),
...
race_ethnic_final %>% group_by(pat_id) %>% filter(n()>1) %>% arrange(pat_id)
# keep row with the least missingness
race_ethnic_unique <- race_ethnic_final %>% 
  mutate(nmiss = rowSums(is.na(.))) %>% 
  group_by(pat_id) %>% 
  filter(nmiss==min(nmiss)) %>% 
  select(-nmiss)
```

```{r check_unique_race_ethnic_2}
check_unique(race_ethnic_unique)
```

Overall coverage of insurance status at baseline in this cohort: `r print_percent(dim(race_ethnic_unique)[1]/dim(pat_cohort)[1])`% of patients.
...
parity_clean <- ob_baseline %>% 
  mutate(parity_final = coalesce(ob_parity, ob_live_births),
         parity_final = ifelse(ob_full_term == 0 & is.na(parity_final), 0, parity_final)) %>%
  select(-contains('ob'))
```
...
Patients with indications of parity information in this cohort: `r print_percent(dim(parity_clean)[1]/dim(pat_cohort)[1])`% of patients.

```{r missing_parity_ages}
pat_cohort %>%
  mutate(parity_flag = ifelse(pat_id %in%
                                parity_clean$pat_id[which(!is.na(parity_clean$parity_final))], 
                              TRUE, FALSE)) %>%
  CreateTableOne(vars='age_20160801', strata='parity_flag', data=.)
```
...
There are `r length(unique(bmi_unique$pat_id))` patients with a clean BMI at baseline which is `r print_percent(length(unique(bmi_unique$pat_id))/dim(pat_cohort)[1])`. 
...
pat_cohort_covars <- pat_cohort %>%
  select(pat_id, age_20160801, hcu_baseline, hcu_fu) %>%
  ...
  left_join(race_ethnic_unique, by='pat_id') %>%
...
summary(tableby(~age_20160801 + hcu_baseline + hcu_fu + parity_final + bmi_baseline, data=pat_cohort_covars),
        labelTranslations = cont_labs)
```

```{r cont_distributions, warning=FALSE}
cont_df <- pat_cohort_covars[,c('age_20160801','hcu_baseline', 'hcu_fu',
                                'parity_final','bmi_baseline')]
...
summary(tableby(~employ_simple_final + insur_status + race_ethnic_final + language_eng_sp + 
                  marital_status_final + nativity, data=pat_cohort_covars, 
...
summary(tableby(~ART_use_baseline + ART_use_fu + HC_status, data=pat_cohort_covars),
...
gg_miss_var(pat_cohort_covars %>% 
              select(-c(pat_id, race_groups, employ_source, employ_margin, employ_clean,
                                         contains('date'), HC_type)), 
            show_pct = TRUE)
```
...
gg_miss_upset(pat_cohort_covars %>% 
                select(-c(pat_id, race_groups, employ_source, employ_margin, 
                          employ_clean, race_final, ethnicity_final, nativity, 
                          contains('date'), HC_type)))
```
...
summary(tableby(complete ~ age_20160801 + bmi_baseline + parity_final + insur_status +
                  race_ethnic_final + language_eng_sp + marital_status_final + employ_simple_final +
                  ART_use_baseline + HC_status, 
