...
```{r}
CreateTableOne(data=pat_unique, var='age_20160801', includeNA=TRUE)
```
...
### Regular relevant care in two years since baseline
...
Type of care in first follow-up encounter:
```{r}
time_to_first_FU %>%
  mutate(`OBGYN general` = case_when(grepl(paste(obgyn_care,collapse="|"),
...
write.csv(pat_unique_final, 'Data/Derived/pat_cohort_w_censoring.csv')
