---
title: "Defining Incident UF from NYULH EHR"
...
procedure_df$location <- grepl(uf_loc_names, tolower(procedure_df$event_name))
## add to other hints
...
print(CreateTableOne(data=uf_dx_hints, includeNA=TRUE,
...
pat_cohort_uf <- pat_cohort %>%
  left_join(uf_dx_hints %>% 
              ...
              select(pat_id, first_dx_date, putative_group, first_confirm_date, first_confirm_type,
                     first_location_date),
            by='pat_id') %>%
  mutate(uf_dx = case_when(!is.na(putative_group)~TRUE, TRUE~FALSE),
...
```{r}
pat_cohort_uf %>%
  mutate(person_years = case_when(is.na(final_censor_date_uf_all)~difftime('2023-08-01',
...
pat_cohort_uf %>%
  ...
  mutate(person_years = case_when(is.na(final_censor_date_uf_confident)~difftime('2023-08-01',
                                                                           index_enc_date,units='days'),
...
```{r save_files}
write.csv(uf_dx_hints, 'Data/Derived/uf_all_hints_20240321.csv', row.names=FALSE)
write.csv(pat_cohort_uf, 'Data/Derived/cohort_w_uf_20240321.csv', row.names=FALSE)
```
