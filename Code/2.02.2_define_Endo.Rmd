---
title: "Defining Incident Endo from NYULH EHR"
...
procedure_hints <- procedure_df %>%
  ...
  summarise(has_imaging = (sum(method == 'Imaging', na.rm=TRUE)>0),
            ...
            first_confirm_date = min(date[which(confirmed_6months |
                                                            confirmed_ever_prevalent)], 
                                               na.rm=TRUE),
            .groups = 'drop')
...
procedure_hints <- procedure_hints %>%
  ...
  rename(first_confirm_type = method) %>%
...
procedure_df$location <- grepl(endo_loc_names, tolower(procedure_df$event_name))
## add to other hints
other_hints <- endo_pat_unique %>%
  left_join(procedure_df %>%
              group_by(pat_id) %>%
              summarise(any_location = (sum(location)>0),
                        first_location_date = min(date[which(location)], na.rm=TRUE)),
            by='pat_id') %>%
  mutate(first_location_date = case_when(is.infinite(first_location_date)~NA,
                                             TRUE~first_location_date))
```
...
print(CreateTableOne(data=endo_dx_hints, includeNA=TRUE,
               vars=c('confident_incident', 'ever_confirmed_6months', 'any_location',
...
pat_cohort_endo <- pat_cohort %>%
  left_join(endo_dx_hints %>% 
              ...
              select(pat_id, first_dx_date, putative_group, first_confirm_date, first_confirm_type,
                     first_location_date),
            by='pat_id') %>%
  mutate(endo_dx = case_when(!is.na(putative_group)~TRUE, TRUE~FALSE),
...
```{r}
pat_cohort_endo %>%
  mutate(person_years = case_when(is.na(final_censor_date_endo_all)~difftime('2023-08-01',
                                                                           index_enc_date,units='days'),
...
pat_cohort_endo %>%
  ...
  mutate(person_years = case_when(is.na(final_censor_date_endo_confident)~difftime('2023-08-01',
                                                                           index_enc_date,units='days'),
...
```{r save_files}
write.csv(endo_dx_hints, 'Data/Derived/endo_all_hints_20240321.csv', row.names=FALSE)
write.csv(pat_cohort_endo, 'Data/Derived/cohort_w_endo_20240321.csv', row.names=FALSE)
```
